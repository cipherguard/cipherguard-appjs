...
import DropdownComponent from 'cipherguard-mad/form/element/dropdown';
...
const EditComponent = Component.extend('cipherguard.component.group.Edit', /** @static */ {
  ...
  loadGroupUser: function(groupUser) {
    ...
    const dropdownComponent = new DropdownComponent($('.js_group_user_is_admin', groupUserTypeSelector)[0], {
      ...
      emptyValue: false,
      modelReference: 'cipherguard.model.GroupUser.is_admin',
      availableValues:  {
        false: __('Member'),
  ...
  '{saveChangesButton.element} click': function() {
    if (this.options.saveChangesButton.state.loaded) {
      ...
      if (validate == true && hasAdmins == true) {
        ...
        MadBus.trigger('cipherguard_loading');
   ...
   */
  '{mad.bus.element} cipherguard.plugin.group.edit.group_loaded': function() {
    /*
   ...
   */
  '{mad.bus.element} cipherguard.group.edit.add_user': function(el, ev) {
    const data = ev.data;
   ...
   */
  '{mad.bus.element} cipherguard.plugin.group.edit.group_users_updated': function(el, ev) {
    const data = ev.data;
  ...
  '{mad.bus.element} group_edit_save_success': function(el, ev) {
    ...
    Group.findOne(findOptions)
      .then(group => {
        ...
        this.state.loaded = true;
        MadBus.trigger('cipherguard_loading_complete');
