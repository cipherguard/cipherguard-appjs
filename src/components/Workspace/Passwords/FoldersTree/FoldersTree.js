...
class FoldersTree extends React.Component {
   ...
   */
  initEventHandlers() {
    document.addEventListener('cipherguard.resources.drag-start', this.handleGridDragStartEvent);
    document.addEventListener('cipherguard.resources.drag-end', this.handleGridDragEndEvent);
  }
  ...
  handleFolderOpenEvent(event, folder) {
    ...
    this.setState({openFolders});
    Plugin.request('cipherguard.plugin.folders.update-local-storage');
  }
  ...
  handleDropTitle(event) {
    ...
    Plugin.send('cipherguard.plugin.folders.open-move-confirmation-dialog', {folders, resources, folderParentId});
