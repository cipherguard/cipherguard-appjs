...
class Grid extends React.Component {
  ...
  handleCopyUsernameClick(ev, resource) {
    ...
    Plugin.send('cipherguard.clipboard', {name, data});
  }

  handleCopyPasswordClick(ev, resource) {
    ev.stopPropagation();
    Plugin.send('cipherguard.plugin.decrypt_secret_and_copy_to_clipboard', resource.id);
  }
  ...
  handleDragStartEvent(event, resource) {
    ...
    trigerEvent.initCustomEvent("cipherguard.resources.drag-start", true, true, draggedItems);
  ...
  handleDragEndEvent() {
    ...
    const trigerEvent = document.createEvent("CustomEvent");
    trigerEvent.initCustomEvent("cipherguard.resources.drag-end", true, true);
    document.dispatchEvent(trigerEvent);
  }

  async favoriteResource(resource) {
    try {
      await Plugin.request('cipherguard.plugin.favorite.add', [resource.id]);
  ...
  async unfavoriteResource(resource) {
    try {
      await Plugin.request('cipherguard.plugin.favorite.delete', [resource.id]);
  ...
  triggerNotification(notification) {
    ...
    event.initCustomEvent("cipherguard_notify", true, true, notification);
