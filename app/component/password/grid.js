...
const GridComponent = Component.extend('cipherguard.component.password.Grid', {
  ...
  defaults: {
     ...
     * @param {HTMLEvent} ev The event which occurred
     */
    '{document} cipherguard.storage.resources.updated': function (el, ev) {
     ...
     * @param {HTMLEvent} ev The event which occurred
     */
    '{mad.bus.element} cipherguard.plugin.resources.select-and-scroll-to': function(el, ev) {
    ...
    afterFilterSelectResource: function (filter) {
      ...
      if (filter.selectedResourceId) {
        ...
        if (resourceIndex !== -1) {
          ...
          if (this.options.isFirstLoad) {
            MadBus.trigger('cipherguard_notify', { status: 'error', title: 'app_passwords_edit_error_not_found' });
          }
        }
      } else {
